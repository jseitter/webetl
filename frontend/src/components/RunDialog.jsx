...
function RunDialog({ open, onClose, sheetId, projectId }) {
    ...
    useEffect(() => {
        if (open) {
            ...
            // Connect to websocket and subscribe to runner output
            websocketService.connect().then(() => {
                const sub = websocketService.subscribeToRunner(
                    sheetId,
                    addMessage
                );
                setSubscription(sub);
                ...
                // Start execution
                fetch(`/api/runner/sheets/${sheetId}/run?projectId=${projectId}`, {
                    method: 'POST'
                });
            });
        }
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
            <DialogTitle>Running Flow</DialogTitle>
            <DialogContent>
                <Box 
                    sx={{ 
                        bgcolor: 'black',
